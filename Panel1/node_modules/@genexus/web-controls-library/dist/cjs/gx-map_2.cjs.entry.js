...
const Map = class {
    constructor(hostRef) {
        ...
        this.gxMapDidLoad = index.createEvent(this, "gxMapDidLoad", 7);
        this.mapClick = index.createEvent(this, "mapClick", 7);
        this.selectionInput = index.createEvent(this, "selectionInput", 7);
        this.selectionChange = index.createEvent(this, "selectionChange", 7);
        this.userLocationChange = index.createEvent(this, "userLocationChange", 7);
    ...
    fitBounds() {
        if (this.markersList.length > 1) {
            const markersGroup = new coordsValidate.FeatureGroup(this.markersList);
    ...
    selectingTypes(mapType) {
        const tileLayerToApply = coordsValidate.tileLayer(mapType, {
    ...
    setMapProvider() {
        ...
        if (this.mapProvider) {
            const tileLayerToApply = coordsValidate.tileLayer(this.mapProvider, {
                maxZoom: this.maxZoom
            });
            tileLayerToApply.addTo(this.map);
    ...
    componentDidLoad() {
        ...
        const coords = coordsValidate.parseCoords(this.center);
        this.maxZoom = this.checkForMaxZoom();
        this.zoom = this.getZoom();
        if (coords !== null) {
            this.map = coordsValidate.createMap(elementVar).setView(coords, this.zoom, this.maxZoom);
        }
        else {
            console.warn("GX warning: Can not read 'center' attribute, default center set (gx-map)", this.element);
            this.map = coordsValidate.createMap(elementVar).setView([0, 0], this.getZoom());
        }
        this.setMapProvider();
    ...
    render() {
        return (index.h(index.Host, null, this.watchPosition && (index.h("gx-map-marker", { "marker-class": "gx-default-user-location-icon", "icon-width": "15", "icon-height": "15", coords: this.userLocationCoords })), this.selectionLayer &&
            (this.isSelectionLayerSlot ? (index.h("slot", { name: "selection-layer-marker" })) : (index.h("gx-map-marker", { "marker-class": "gx-default-selection-layer-icon", "icon-width": "30", "icon-height": "30", coords: this.centerCoords }))), index.h("div", { class: "gxMapContainer" }, index.h("div", { class: "gxMap" }))));
    }
...
const mapMarkerCss = ".gx-line-clamp{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:var(--max-lines);overflow:hidden;max-height:var(--max-height)}.line-measuring{visibility:hidden;position:absolute}gx-map-marker{display:none}gx-map-marker[hidden]{display:none !important}gx-map-marker[hidden][invisible-mode=keep-space]{display:none !important;visibility:hidden}";

const MAX_POPUP_SIZE_FACTOR = 0.83;
const MapMarker = class {
    constructor(hostRef) {
        index.registerInstance(this, hostRef);
        this.gxMapMarkerDidLoad = index.createEvent(this, "gxMapMarkerDidLoad", 7);
        this.gxMapMarkerUpdate = index.createEvent(this, "gxMapMarkerUpdate", 7);
        this.gxMapMarkerDeleted = index.createEvent(this, "gxMapMarkerDeleted", 7);
        /**
    ...
    setupMarker(coords) {
        this.markerInstance = coordsValidate.marker(coords, {
            icon: coordsValidate.divIcon({
    ...
    componentDidUpdate() {
        ...
        this.markerInstance.setIcon(coordsValidate.divIcon({
            className: this.markerClass,
            iconAnchor: [this.getHalfSizes().width, this.iconHeight],
            popupAnchor: [0, -this.getHalfSizes().height],
            iconSize: [this.iconWidth, this.iconHeight],
    ...
    render() {
        return (index.h("div", { class: "popup-data-container" }, index.h("slot", null)));
    }
    get element() { return index.getElement(this); }
};
...
exports.gx_map = Map;
exports.gx_map_marker = MapMarker;
