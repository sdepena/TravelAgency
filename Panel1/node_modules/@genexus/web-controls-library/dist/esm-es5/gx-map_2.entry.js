...
var Map = /** @class */ (function () {
    ...
    Map.prototype.userLocationHandler = function () {
        this.userLocationChange.emit(this.userLocationCoords);
    };
    Map.prototype.selectionLayerHandler = function () {
        this.registerSelectionLayerEvents();
    };
    Map.prototype.onMapMarkerDidLoad = function (event) {
        var _this = this;
        var markerElement = event.target;
        var markerV = event.detail;
        if (this.map) {
            markerV.addTo(this.map);
        }
        else {
            this.element.addEventListener("gxMapDidLoad", function () {
                markerV.addTo(_this.map);
            });
        }
        if (this.selectionLayer) {
            var slot = this.getSelectionMarkerSlot();
            if (slot.exist) {
        ...
        markerElement.addEventListener("gxMapMarkerDeleted", function () {
            _this.onMapMarkerDeleted(markerV);
        });
    };
    Map.prototype.onMapLineDidLoad = function (event) {
        var _this = this;
        var lineElement = event.target;
        var lineV = event.detail;
        if (this.map) {
            lineV.addTo(this.map);
        }
        else {
            this.element.addEventListener("gxMapDidLoad", function () {
                lineV.addTo(_this.map);
            });
        }
        lineElement.addEventListener("gxMapLineDeleted", function () {
            _this.onMapLineDeleted(lineV);
        });
    };
    Map.prototype.addMapListener = function (eventToListen, callbackFunction) {
        this.map.on(eventToListen, callbackFunction);
    };
    Map.prototype.removeMapListener = function (eventToListen, callbackFunction) {
        this.map.off(eventToListen, callbackFunction);
    };
    Map.prototype.checkForMaxZoom = function () {
        return this.maxZoom < 20 ? this.maxZoom : RECOMMENDED_MAX_ZOOM;
    };
    Map.prototype.fitBounds = function () {
        if (this.markersList.length > 1) {
            var markersGroup = new FeatureGroup(this.markersList);
            this.map.fitBounds(markersGroup.getBounds());
        }
        else if (this.markersList.length === 1) {
            var marker_1 = this.markersList[0];
            var markerCoords = [marker_1._latlng.lat, marker_1._latlng.lng];
    ...
    Map.prototype.getSelectionMarkerSlot = function () {
        var slot = this.element.querySelector("[slot='selection-layer-marker']");
        return { exist: slot !== null, elem: slot };
    };
    Map.prototype.onMapMarkerDeleted = function (marker) {
        var i = 0;
        marker.remove();
    ...
    Map.prototype.onMapLineDeleted = function (line) {
        var i = 0;
        line.remove();
    ...
    Map.prototype.updateSelectionMarkerPosition = function () {
    ...
    Map.prototype.registerSelectionLayerEvents = function () {
        var _this = this;
        if (this.selectionLayer) {
            var moveBehaivor = {
                eventTrigger: "move",
                callbackFunction: function () {
                    _this.updateSelectionMarkerPosition();
                    _this.selectionInput.emit(_this.centerCoords);
                }
            };
            var moveEndBehaivor = {
                ...
                callbackFunction: function () {
                    _this.updateSelectionMarkerPosition();
                    _this.selectionChange.emit(_this.centerCoords);
                }
            };
            if (this.selectionLayer) {
    ...
    Map.prototype.selectingTypes = function (mapType) {
        var tileLayerToApply = tileLayer(mapType, {
            maxZoom: this.maxZoom
        });
        tileLayerToApply.addTo(this.map);
        this.mapProviderApplied = tileLayerToApply;
    };
    Map.prototype.setMapProvider = function () {
        if (this.mapProviderApplied && this.tileLayerApplied) {
            this.tileLayerApplied.removeFrom(this.map);
        }
        if (this.mapProvider) {
            var tileLayerToApply = tileLayer(this.mapProvider, {
                maxZoom: this.maxZoom
            });
            tileLayerToApply.addTo(this.map);
    ...
    Map.prototype.setUserLocation = function (_a) {
        var coords = _a.coords;
        this.userLocationCoords = coords.latitude + ", " + coords.longitude;
    };
    Map.prototype.componentWillLoad = function () {
        if (this.watchPosition) {
            this.watchPositionId = watchPosition(this.setUserLocation.bind(this), function (err) { return console.error(err); }, {
    ...
    Map.prototype.componentDidLoad = function () {
        var _this = this;
        var elementVar = this.element.querySelector(".gxMap");
        var coords = parseCoords(this.center);
        this.maxZoom = this.checkForMaxZoom();
        this.zoom = this.getZoom();
        if (coords !== null) {
            this.map = createMap(elementVar).setView(coords, this.zoom, this.maxZoom);
        }
        else {
            console.warn("GX warning: Can not read 'center' attribute, default center set (gx-map)", this.element);
            this.map = createMap(elementVar).setView([0, 0], this.getZoom());
        }
        this.setMapProvider();
        ...
        this.addMapListener("popupopen", function (e) {
            var px = this.project(e.target._popup._latlng);
            px.y -= e.target._popup._container.clientHeight / 2;
            this.panTo(this.unproject(px), { animate: true });
        });
        this.addMapListener("click", function (ev) {
            _this.mapClick.emit(ev.latlng);
        });
    };
    Map.prototype.componentDidUpdate = function () {
        var maxZoom = this.checkForMaxZoom();
        this.setMapProvider();
        if (this.selectionLayer) {
            this.fitBounds();
        }
    ...
    Map.prototype.componentDidUnload = function () {
        navigator.geolocation.clearWatch(this.watchPositionId);
    };
    Map.prototype.render = function () {
        return (h(Host, null, this.watchPosition && (h("gx-map-marker", { "marker-class": "gx-default-user-location-icon", "icon-width": "15", "icon-height": "15", coords: this.userLocationCoords })), this.selectionLayer &&
            (this.isSelectionLayerSlot ? (h("slot", { name: "selection-layer-marker" })) : (h("gx-map-marker", { "marker-class": "gx-default-selection-layer-icon", "icon-width": "30", "icon-height": "30", coords: this.centerCoords }))), h("div", { class: "gxMapContainer" }, h("div", { class: "gxMap" }))));
    };
    ...
    Object.defineProperty(Map, "watchers", {
        get: function () {
            return {
...
Map.style = mapCss;
var mapMarkerCss = ".gx-line-clamp{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:var(--max-lines);overflow:hidden;max-height:var(--max-height)}.line-measuring{visibility:hidden;position:absolute}gx-map-marker{display:none}gx-map-marker[hidden]{display:none !important}gx-map-marker[hidden][invisible-mode=keep-space]{display:none !important;visibility:hidden}";
var MAX_POPUP_SIZE_FACTOR = 0.83;
var MapMarker = /** @class */ (function () {
    function MapMarker(hostRef) {
        registerInstance(this, hostRef);
        this.gxMapMarkerDidLoad = createEvent(this, "gxMapMarkerDidLoad", 7);
        this.gxMapMarkerUpdate = createEvent(this, "gxMapMarkerUpdate", 7);
        this.gxMapMarkerDeleted = createEvent(this, "gxMapMarkerDeleted", 7);
        /**
    ...
    MapMarker.prototype.getHalfSizes = function () {
        var halfIconSizes = {
    ...
    MapMarker.prototype.setupMarker = function (coords) {
    ...
    MapMarker.prototype.setPopup = function () {
        var popupContainerEl = this.element.querySelector("[class='popup-data-container']");
    ...
    MapMarker.prototype.componentDidLoad = function () {
    ...
    MapMarker.prototype.componentDidUpdate = function () {
        var coords = parseCoords(this.coords);
        if (coords !== null) {
    ...
    MapMarker.prototype.componentDidUnload = function () {
        this.gxMapMarkerDeleted.emit(this.markerInstance);
    };
    MapMarker.prototype.render = function () {
        return (h("div", { class: "popup-data-container" }, h("slot", null)));
    };
    Object.defineProperty(MapMarker.prototype, "element", {
        get: function () { return getElement(this); },
        enumerable: false,
        configurable: true
    });
...
export { Map as gx_map, MapMarker as gx_map_marker };
